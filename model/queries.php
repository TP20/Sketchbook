<?php
 ...
 Class Queries {
    ...
    public static function recentlyViewed(){
        include $_SERVER['DOCUMENT_ROOT']."/VidTube_organize_local/model/dbconnect.php";
        $sql = "select distinct image, user, title, max(id) from images group by image order by max(id) desc limit 10;";
...
//         $sql = "Insert into registration values ('".$fName."','".$lName."','".$location."',".$pNumber.",'".$username."','".$password."')";
     ...
     public static function imageViewedDates ($image) {
        // include $_SERVER['DOCUMENT_ROOT']."/VidTube_organize/model/dbconnect.php";
        // $sql = "select * from images where user='".$user."' group by image;";
        // echo "USER-----------------------".$user;
        // $results = $conn -> query($sql);
        // return $results;
        include $_SERVER['DOCUMENT_ROOT']."/VidTube_organize_local/model/dbconnect.php";
        $sql = "SELECT distinct image, monthname(date), count(*) as Viewed FROM `images` WHERE image ='".$image."' group by month(date)";
        // $sql = "select * from images where user='".$user."' group by image;";
        $results = $conn -> query($sql);
        return $results;
    }
